...
def main() -> int:
    ...
    for model_set in ("v6-medium", "v6-tiny"):
        for fp16 in (False, True):
            with benchmark_engine(model_set, use_fp16=fp16) as engine:
                ...
                print(
                    f"{model_set:10} fp16={str(fp16):5} rec {ms:7.1f} ms for "
                    f"{len(crops)} crops ({pixels / 1e6:.2f} MPix) = "
                    f"{ms / len(crops):5.2f} ms/crop, "
                    f"{ms / (pixels / 1e6):6.1f} ms/MPix"
                )

    # Dispatch overhead measured directly: the same crop, N times, vs one crop N times wider.
    with benchmark_engine("v6-medium", use_fp16=False) as engine:
        crops = require_text_crops(engine, rgb)
