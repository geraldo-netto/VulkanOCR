...
def recognise_all(engine, crops):
    ...
    return [recognise_patch(engine._runtime, engine._rec, c, engine._characters, engine._models.blobs, offset) for c in crops]
...
for model_set in ("v6-medium", "v6-tiny"):
    for fp16 in (False, True):
        ...
        print(f"{model_set:10} fp16={str(fp16):5} rec {ms:7.1f} ms for {len(crops)} crops "
              f"({pixels/1e6:.2f} MPix) = {ms/len(crops):5.2f} ms/crop, {ms/(pixels/1e6):6.1f} ms/MPix")
...
offset = 0 if engine._models.dictionary_includes_blank else 1
recognise_patch(engine._runtime, engine._rec, narrow, engine._characters, engine._models.blobs, offset)
start = time.perf_counter()
for _ in range(20):
    recognise_patch(engine._runtime, engine._rec, narrow, engine._characters, engine._models.blobs, offset)
per_call = (time.perf_counter() - start) / 20 * 1000
wide = np.tile(narrow, (1, 20, 1))
recognise_patch(engine._runtime, engine._rec, wide, engine._characters, engine._models.blobs, offset)
start = time.perf_counter()
for _ in range(3):
    recognise_patch(engine._runtime, engine._rec, wide, engine._characters, engine._models.blobs, offset)
wide_ms = (time.perf_counter() - start) / 3 * 1000
print(f"\nsmallest crop ({narrow.shape[1]}px wide): {per_call:.1f} ms each")
print(f"the same content 20x wider ({wide.shape[1]}px): {wide_ms:.1f} ms once "
      f"vs {per_call*20:.1f} ms as 20 calls -> per-call overhead ~{(per_call*20-wide_ms)/20:.2f} ms")
