...
rows = []
subprocess.run(["tesseract", str(corpus / cases[0]["image"]), "stdout", "-l", "eng", "--psm", psm],
...
for case in cases:
    ...
    row = {"id": case["id"], "variant": case["variant"], "ms": elapsed,
           "observed": " ".join(observed.split()), **score(" ".join(case["lines"]), observed)}
    rows.append(row)
...
out.write_text(json.dumps({"engine": f"tesseract-5.3.4/psm{psm}", "device": "CPU", "rows": rows}, indent=2, ensure_ascii=False))
