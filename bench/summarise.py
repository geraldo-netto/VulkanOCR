...
print(f"{'engine':34} {'CER':>7} {'WER':>7} {'exact':>7} {'ms p50':>8} {'ms p95':>8} {'total s':>8}")
print("-" * 86)
summaries = []
for path in files:
    ...
    print(f"{doc['engine']:34} {cer:7.4f} {wer:7.4f} {exact:6.0%} {p50:8.1f} {p95:8.1f} {total:8.1f}")

print()
variants = sorted({r["variant"] for _n, _d, *_rest, rows in summaries for r in rows})
header = f"{'variant':20}" + "".join(f"{name.split('/')[-1][:13]:>15}" for name, *_ in summaries)
print(header)
print("-" * len(header))
for variant in variants:
    ...
    for *_meta, rows in summaries:
        ...
        cer = sum(r["char_distance"] for r in subset) / max(1, sum(r["char_length"] for r in subset))
        line += f"{cer:15.3f}"
    print(line)
