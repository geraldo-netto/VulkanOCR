...
def logits_for(engine, strip):
    ...
    mat = engine._runtime.Mat.from_pixels(np.ascontiguousarray(strip),
        engine._runtime.Mat.PixelType.PIXEL_RGB2BGR, width, height)
...
def read_packed(engine, crops, offset):
    ...
    for left, right in spans:
        lo = int(round(left * scale)); hi = max(int(round(right * scale)), lo + 1)
...
for threshold in (0, 96, 160, 256, 10_000):
    ...
    def run():
        ...
        if narrow:
            for text, index in zip(read_packed(engine, [crops[i] for i in narrow], offset), narrow, strict=True):
                got[index] = text
    ...
    print(f"  pack crops <= {threshold:5} px ({len(narrow):2} packed, {len(wide):2} single): "
