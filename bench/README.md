...
Everything here runs against `../mvp/ocr_engine`. Nothing is installed into
OmniTensor and nothing in `../mvp` was changed to make these run.

## The corpus

`make_corpus.py` renders 5 texts × 11 degradations = 55 images with exact
...
`metrics.py` scores a read: CER and WER as total edit distance over total
...
```sh
../.venv/bin/python make_corpus.py /tmp/ocrcorpus
../.venv/bin/python run_spike.py /tmp/ocrcorpus v6-medium
python3 run_tesseract.py /tmp/ocrcorpus 6
<paddle venv>/bin/python run_paddle.py /tmp/ocrcorpus
python3 summarise.py /tmp
```
...
Dense UI page (`../sample-applet.png`, 585×770): Tesseract 264 ms wall / 827 ms
...
`gpu_fdinfo.py` reads this process's own amdgpu counters: **+1314 ms of
...
`phase_split.py` shows 748 of 752 ms inside the nets — 39 ms detection (one
...
| --- | --- | --- |
| concurrent extractors, 2–16 threads | `batch_poc.py` | 0.96–0.99× — no gain. The Python binding serialises; text identical, so it is safe, just pointless |
| pack every crop into one wide strip | `batch_strip.py` | 1.01–1.05×, and **only 37/46 lines identical** — the encoder mixes across the strip, so the decode is wrong |
| pack only crops narrower than N | `narrow_batch.py` | 664–795 ms against a 734 ms baseline, inside the noise, still 44/46 identical |

`where_time_goes.py` explains it. Per-call overhead is ~5.1 ms: the smallest
